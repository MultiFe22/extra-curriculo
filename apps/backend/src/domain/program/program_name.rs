...
impl ProgramName {
    /// Returns an instance of `ProgramName` if the input satisfies all
    /// our validation constraints on subscriber names.
    /// It panics otherwise.
    pub fn parse(s: String) -> Result<ProgramName, String> {
...
mod tests {
    ...
    fn a_256_grapheme_long_name_is_valid() {
        ...
        assert_ok!(ProgramName::parse(name));
    }
    #[test]
    ...
    fn string_starting_or_ending_with_whitespace_is_rejected() {
        ...
        assert_err!(ProgramName::parse(name));
        assert_err!(ProgramName::parse(name2));
        assert_err!(ProgramName::parse(name3));
    }
