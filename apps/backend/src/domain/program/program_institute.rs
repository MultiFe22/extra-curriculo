...
#[derive(Debug)]
pub struct ProgramInstitute(String);

impl ProgramInstitute {
    /// Returns an instance of `ProgramInstitute` if the input satisfies all
    /// our validation constraints on subscriber names.
    /// It panics otherwise.
    pub fn parse(s: String) -> Result<ProgramInstitute, String> {
        ...
        {
            Err(format!("{} is not a valid institute name.", s))
        } else {
            Ok(Self(s))
        }
    }
...
impl AsRef<str> for ProgramInstitute {
    fn as_ref(&self) -> &str {
        &self.0
    }
}
...
mod tests {
    ...
    fn string_starting_or_ending_with_whitespace_is_rejected() {
        ...
        assert_err!(ProgramInstitute::parse(name));
        assert_err!(ProgramInstitute::parse(name2));
        assert_err!(ProgramInstitute::parse(name3));
    }
