...
#[derive(Debug)]
pub struct ProgramCollege(String);

impl ProgramCollege {
    /// Returns an instance of `ProgramCollege` if the input satisfies all
    /// our validation constraints on subscriber names.
    /// It panics otherwise.
    pub fn parse(s: String) -> Result<ProgramCollege, String> {
        ...
        {
            Err(format!("{} is not a valid college name.", s))
        } else {
            Ok(Self(s))
        }
    }
...
impl AsRef<str> for ProgramCollege {
    fn as_ref(&self) -> &str {
        &self.0
    }
}
...
mod tests {
    ...
    fn string_starting_or_ending_with_whitespace_is_rejected() {
        ...
        assert_err!(ProgramCollege::parse(name));
        assert_err!(ProgramCollege::parse(name2));
        assert_err!(ProgramCollege::parse(name3));
    }
