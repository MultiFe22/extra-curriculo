...
impl TestApp {
    ...
    pub async fn get_program(&self, program_id: Uuid) -> reqwest::Response {
        self.api_client
            .get(&format!("{}/programs/{}", &self.address, program_id))
            .send()
            .await
            .expect("Failed to execute request.")
    }
    ...
    pub async fn post_tag(&self, tag: Value) -> reqwest::Response {
        self.api_client
    ...
    pub async fn get_tag(&self, tag_id: Uuid) -> reqwest::Response {
        self.api_client
            .get(&format!("{}/tags/{}", &self.address, tag_id))
            .send()
            .await
            .expect("Failed to execute request.")
    }
    ...
    pub async fn post_logout(&self, cookie: &str) -> reqwest::Response {
        self.api_client
...
impl TestUser {
    ...
    async fn store(&self, pool: &PgPool) {
        ...
        // We don't care about the exact Argon2 parameters here because it's for testing purposes
        let password_hash = Argon2::new(
            Algorithm::Argon2id,
            Version::V0x13,
            Params::new(20000, 2, 1, None).unwrap(),
        )
